...
public class MobKillPirateListener implements Listener {

    private final Main plugin;

    public MobKillPirateListener(Main plugin) {
        this.plugin = plugin;
    }
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        if (event.getEntity().getKiller() != null) {
            ...
            if (event.getEntityType() == EntityType.EVOKER
                ...
                if (hasSpecificItem(player, "§5§l✤ Épée du Pirate  ✤")) {
    ...
    private void handleMobKill(Player player) {
        plugin.getEconomy().depositPlayer(player, 20.0);
    }
