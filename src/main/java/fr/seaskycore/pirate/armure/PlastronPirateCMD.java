...
import fr.seaskycore.pirate.armure.listener.PlastronPirateListener;
...
public class PlastronPirateCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("plastronpirate")) {
            if (args.length < 1 || args.length > 2) {
                sender.sendMessage("§cUtilisation incorrecte. Utilisez /plastronpirate <joueur> [quantité]");
                return true;
            }
            ...
            for (int i = 0; i < quantity; i++) {
                targetPlayer.getInventory().addItem(PlastronPirateListener.createPlastronPirate());
            }

            // Message de confirmation
            sender.sendMessage("§eVous avez donné §f" + quantity + " §cPlastron Pirate à §e" + targetPlayer.getName());
            return true;
        }
