...
import fr.seaskycore.caisse.lunaire.armure.listener.BotteLunaireListener;
...
public class BotteLunaireCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("bottelunaire")) {
            if (args.length < 1 || args.length > 2) {
                sender.sendMessage("§cUtilisation incorrecte. Utilisez /bottelunaire <joueur> [quantité]");
                return true;
            }
            ...
            for (int i = 0; i < quantity; i++) {
                targetPlayer.getInventory().addItem(BotteLunaireListener.createLunaireBotte());
            }
