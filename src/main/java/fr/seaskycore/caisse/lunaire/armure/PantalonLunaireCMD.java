...
import fr.seaskycore.caisse.lunaire.armure.listener.PantalonLunaireListener;
import fr.seaskycore.caisse.lunaire.armure.listener.PlastronLunaireListener;
...
public class PantalonLunaireCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("pantalonlunaire")) {
            if (args.length < 1 || args.length > 2) {
                sender.sendMessage("§cUtilisation incorrecte. Utilisez /pantalonlunaire <joueur> [quantité]");
                return true;
            }
            ...
            for (int i = 0; i < quantity; i++) {
                targetPlayer.getInventory().addItem(PantalonLunaireListener.createLunairePantalon());
            }
