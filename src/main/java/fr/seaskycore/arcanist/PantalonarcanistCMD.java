...
import fr.seaskycore.arcanist.listener.PantalonArcanistListener;
import fr.seaskycore.arcanist.listener.PlastronArcanistListener;
...
public class PantalonarcanistCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("pantalonarcanist")) {
            if (args.length < 1 || args.length > 2) {
                sender.sendMessage("§cUtilisation incorrecte. Utilisez /pantalonarcanist <joueur> [quantité]");
                return true;
            }
            ...
            for (int i = 0; i < quantity; i++) {
                targetPlayer.getInventory().addItem(PlastronArcanistListener.createPlastronCrcanist());
            }
