...
public class PlastronArcanistListener {

    public static ItemStack createPlastronCrcanist() {
        ...
        meta.setDisplayName("Pantalon du Arcanist");
    ...
    public static boolean isCasqueFarmeur(ItemStack item) {
        if (item != null && item.getType() == Material.LEATHER_HELMET && item.hasItemMeta()) {
            ...
            if (meta.getDisplayName() != null && meta.getDisplayName().equals("Pantalon du Arcanist")) {
                return true;
            }
        }
        return false;
