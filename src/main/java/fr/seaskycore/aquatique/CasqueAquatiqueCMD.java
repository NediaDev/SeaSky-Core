...
public class CasqueAquatiqueCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("casqueaquatique")) {
            if (args.length < 1 || args.length > 2) {
                sender.sendMessage("§cUtilisation incorrecte. Utilisez /casqueaquatique <joueur> [quantité]");
                return true;
            }
            ...
            for (int i = 0; i < quantity; i++) {
                targetPlayer.getInventory().addItem(CasqueAquatiqueListener.createCasqueAquatique());
            }

            // Message de confirmation
            sender.sendMessage("§eVous avez donné §f" + quantity + " §cCasque Aquatique à §e" + targetPlayer.getName());
            return true;
        }
