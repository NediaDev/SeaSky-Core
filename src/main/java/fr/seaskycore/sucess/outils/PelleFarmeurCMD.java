...
import fr.seaskycore.sucess.outils.listener.PelleFarmeurListener;
...
public class PelleFarmeurCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("pellefarmeur")) {
            if (args.length < 1 || args.length > 2) {
                sender.sendMessage("§cUtilisation incorrecte. Utilisez /pellefarmeur <joueur> [quantité]");
                return true;
            }
            ...
            for (int i = 0; i < quantity; i++) {
                targetPlayer.getInventory().addItem(PelleFarmeurListener.createPelle());
            }

            // Message de confirmation
            sender.sendMessage("§eVous avez donné §f" + quantity + " §cPelle du Farmeur à §e" + targetPlayer.getName());
            return true;
        }
