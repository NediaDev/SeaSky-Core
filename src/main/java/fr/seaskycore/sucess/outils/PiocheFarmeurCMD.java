...
import fr.seaskycore.sucess.outils.listener.PiocheFarmeurListener;
...
public class PiocheFarmeurCMD implements CommandExecutor  {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("piochefarmeur")) {
            if (args.length < 1 || args.length > 2) {
                sender.sendMessage("§cUtilisation incorrecte. Utilisez /piochefarmeur <joueur> [quantité]");
                return true;
            }
            ...
            for (int i = 0; i < quantity; i++) {
                targetPlayer.getInventory().addItem(PiocheFarmeurListener.createPioche());
            }

            // Message de confirmation
            sender.sendMessage("§eVous avez donné §f" + quantity + " §cPioche du Farmeur à §e" + targetPlayer.getName());
            return true;
        }
