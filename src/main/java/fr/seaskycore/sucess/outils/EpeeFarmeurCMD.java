...
import fr.seaskycore.sucess.outils.listener.EpeeFarmeurListener;
...
public class EpeeFarmeurCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("epeefarmeur")) {
            if (args.length < 1 || args.length > 2) {
                sender.sendMessage("§cUtilisation incorrecte. Utilisez /epeefarmeur <joueur> [quantité]");
                return true;
            }
            ...
            for (int i = 0; i < quantity; i++) {
                targetPlayer.getInventory().addItem(EpeeFarmeurListener.createEpee());
            }

            // Message de confirmation
            sender.sendMessage("§eVous avez donné §f" + quantity + " §cEpee du Farmeur à §e" + targetPlayer.getName());
            return true;
        }
