...
public class CasqueFarmeurCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("casquefarmeur")) {
            if (args.length < 1 || args.length > 2) {
                sender.sendMessage("§cUtilisation incorrecte. Utilisez /casquefarmeur <joueur> [quantité]");
                return true;
            }
            ...
            for (int i = 0; i < quantity; i++) {
                targetPlayer.getInventory().addItem(CasqueFarmeurListener.createFarmeurCasque());
            }
