...
import fr.seaskycore.sucess.armure.listener.PantalonFarmeurListener;
...
public class PantalonFarmeurCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("pantalonfarmeur")) {
            if (args.length < 1 || args.length > 2) {
                sender.sendMessage("§cUtilisation incorrecte. Utilisez /pantalonfarmeur <joueur> [quantité]");
                return true;
            }
            ...
            for (int i = 0; i < quantity; i++) {
                targetPlayer.getInventory().addItem(PantalonFarmeurListener.createPantalon());
            }

            // Message de confirmation
            sender.sendMessage("§eVous avez donné §f" + quantity + " §cPantalon du Farmeur à §e" + targetPlayer.getName());
            return true;
        }
